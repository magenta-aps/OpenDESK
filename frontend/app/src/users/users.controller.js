...
function UsersController ($mdDialog, toastService, usersService, $translate) {
  ...
  function showUserDialog (ev, user) {
    if (user)
      usersService.getUserInfo(user.userName).then(function (userInfo) {
  ...
  function _showUserDialog (ev, userInfo) {
    $mdDialog.show({
      ...
      template: userCRUDDialogTemplate,
  ...
  function getAllSystemUsers (query) {
    var filter = query || ''
    return usersService.getPeople(filter).then(function (response) {
